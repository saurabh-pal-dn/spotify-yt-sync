...
spotify_credentials = json.load(open('client_secret_spotify.json'))
...
def get_playlists_info(access_token):
    ...
    r = requests.get(
        BASE_URL + f'users/{spotify_credentials["USER_ID"]}/playlists', headers=headers)
    return (r.json())
...
def get_playlist_info(access_token, playlist_id):
    headers = get_header(access_token)
    r = requests.get(
        BASE_URL + f'playlists/{playlist_id}', headers=headers)
    return (r.json())
...
def handle_each_playlist(playlist_name: str, playlist_info, youtube_playlist: Playlist):
    ...
    for track in tracks:
        video_id_from_youtube_search: List[str] = youtube_client.search_name_on_youtube_and_get_video_id(
            query=track.name)
...
if __name__ == "__main__":
    ...
    for playlist in playlists:
        youtube_playlists: List[Playlist] = filter(
            lambda v: v.name == playlist.name, all_youtube_playlists)
        ...
        handle_each_playlist(playlist_name=playlist.name,
